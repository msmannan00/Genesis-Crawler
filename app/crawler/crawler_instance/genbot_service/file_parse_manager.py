...
from crawler.crawler_instance.local_shared_model.image_model import image_model_init, image_model_list
...
class file_parse_manager:
    ...
    def __is_image_favourable(self, p_list, p_request_model: url_model):
        ...
        for m_url in p_list:
            try:
                if (p_request_model.m_depth == 0 and len(m_filtered_list) >= 15) or (p_request_model.m_depth == 1 and len(m_filtered_list) >= 5) or m_porn_image_count > 5:
                    break
                ...
                if self.__m_duplication_url_handler.validate_duplicate(m_url) is False:
                    ...
                    if m_status:
                        ...
                        if len(m_file_type) > 4 or m_file_type == "gif" or m_content_type != "image" or len(m_response.content) < 15000 or " html" in str(m_response.content):
                            continue
        ...
        return image_model_list(m_images=m_filtered_list), m_porn_image_count

    def parse_static_files(self, p_images, p_documents, p_videos, p_content_type, p_local_parser, p_request_model: url_model):
        if p_local_parser is True:
            m_images = p_images
            m_porn_image_count = 0
        else:
            m_images, m_porn_image_count = self.__is_image_favourable(p_images, p_request_model)
