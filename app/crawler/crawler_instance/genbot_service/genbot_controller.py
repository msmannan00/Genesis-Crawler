...
class i_crawl_controller(request_handler):
        ...
        if m_response is True:
            ...
            if m_redirected_url == m_redirected_requested_url or m_redirected_url != m_redirected_requested_url and self.__m_url_duplication_handler.validate_duplicate(m_redirected_url) is False:
                ...
                if m_parsed_model.m_validity_score >= 15 and (len(m_parsed_model.m_content) > 0) and m_response:
                    ...
                    m_parsed_model = self.__m_html_parser.on_parse_files(m_parsed_model, m_images)
    ...
    def start_crawler_instance(self, p_request_url):
        ...
        while len(self.__m_unparsed_url) > 0:
            ...
            m_parsed_model = self.__trigger_url_request(item)

            if m_parsed_model and item.m_depth < CRAWL_SETTINGS_CONSTANTS.S_MAX_ALLOWED_DEPTH and len(
            ...
            mongo_controller.get_instance().invoke_trigger(MONGO_CRUD.S_UPDATE, [MONGODB_COMMANDS.S_UPDATE_INDEX, [
                helper_method.on_clean_url(item.m_url), self.__m_parsed_url, self.__m_unparsed_url], [True]])
