...
class genbot_controller(request_handler):
    ...
    def init(self, p_url):
        self.__m_host_failure_count = int(redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_GET_INT, [
            REDIS_KEYS.HOST_FAILURE_COUNT + p_url, 0, 60 * 60 * 24 * 5]))
        self.__m_host_score = redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_GET_FLOAT,
                                                                             [REDIS_KEYS.RAW_HTML_SCORE + p_url, -1,
                                                                              60 * 60 * 24 * 10])
        self.__m_first_time = False
        self.__m_Low_yield = bool(redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_GET_BOOL, [
            REDIS_KEYS.HOST_LOW_YIELD_COUNT + p_url, False, 60 * 60 * 24 * 5]))
        ...
        m_mongo_response = mongo_controller.get_instance().invoke_trigger(MONGO_CRUD.S_READ,
                                                                          [MONGODB_COMMANDS.S_GET_INDEX,
                                                                           [m_requested_url], [None]])
        m_unparsed_url = []
        ...
        for m_data in m_mongo_response:
            ...
            self.__m_html_parser.on_static_parser_init(m_data["document_url_parsed"], m_data["image_url_parsed"],
                                                       m_data["video_url_parsed"])
            break
    ...
    def __check_content_duplication(self, p_parsed_model):
        ...
        if m_score <= 0.7:
            ...
            log.g().w(str(self.__task_id) + " : " + str(
                self.__m_tor_id) + " : " + MANAGE_CRAWLER_MESSAGES.S_DUPLICATE_CONTENT + " : " + str(m_score))
            return True
    ...
    def validate_duplicate_host_url(self, p_request_url, p_raw_html, p_full_content):
        ...
        if p_raw_html is not None:
            ...
            m_hashed_duplication_status = redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_GET_STRING,
                                                                                         [m_hash_duplication_key, None,
                                                                                          60 * 60 * 24 * 5])
            if m_hashed_duplication_status is not None and self.__m_host_score == -1:
                redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_SET_FLOAT,
                                                               [REDIS_KEYS.RAW_HTML_SCORE + p_request_url, 1,
                                                                60 * 60 * 24 * 10])
            ...
            if self.__m_host_score == -1 and m_hashed_duplication_status is None:
                ...
                files = redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_GET_LIST,
                                                                       [REDIS_KEYS.RAW_HTML_CODE + p_request_url[7:8],
                                                                        None, 60 * 60 * 24 * 10])
                m_max_similarity = 0
                for html in files:
                ...
                redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_SET_LIST,
                                                               [REDIS_KEYS.RAW_HTML_CODE + p_request_url[7:8],
                                                                p_raw_html, None])
                redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_SET_FLOAT,
                                                               [REDIS_KEYS.RAW_HTML_SCORE + p_request_url,
                                                                m_max_similarity, 60 * 60 * 24 * 10])

            if m_hashed_duplication_status is None:
                redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_SET_STRING,
                                                               [m_hash_duplication_key, p_request_url,
                                                                60 * 60 * 24 * 5])
            elif self.__m_host_score == -1:
                log.g().w(str(self.__task_id) + " : " + str(
                    self.__m_tor_id) + " : " + MANAGE_CRAWLER_MESSAGES.S_DUPLICATE_HOST_HASH + " : " + str(
                    p_request_url) + " : " + str(m_hashed_duplication_status))
            ...
            if m_max_similarity < 0.90:
                ...
                log.g().w(str(self.__task_id) + " : " + str(
                    self.__m_tor_id) + " : " + MANAGE_CRAWLER_MESSAGES.S_DUPLICATE_HOST_CONTENT + " : " + str(
                    p_request_url) + " : " + str(m_max_similarity))
    ...
    def __trigger_url_request(self, p_request_model: url_model):
        try:
            log.g().i(str(self.__task_id) + " : " + str(
                self.__m_tor_id) + " : " + MANAGE_CRAWLER_MESSAGES.S_PARSING_STARTING + " : " + p_request_model.m_url)
            ...
            if m_response is True:
                ...
                if not self.__m_host_duplication_validated:
                    self.__m_host_failure_count = 0
                    redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_SET_INT, [
                        REDIS_KEYS.HOST_FAILURE_COUNT + p_request_model.m_url, self.__m_host_failure_count,
                        60 * 60 * 24 * 5])
                ...
                if m_redirected_url == m_redirected_requested_url or m_redirected_url != m_redirected_requested_url and self.__m_url_duplication_handler.validate_duplicate(
                        m_redirected_url) is False:
                    ...
                    if m_parsed_model.m_validity_score >= 15 and (len(m_parsed_model.m_content) > 0) and m_response:
                        if not self.__m_host_duplication_validated:
                            status = self.validate_duplicate_host_url(p_request_model.m_url, m_raw_html,
                                                                      m_parsed_model.m_extended_content)
                        else:
                            status = True

                        if status:
                            log.g().s(str(self.__task_id) + " : " + str(
                                self.__m_tor_id) + " : " + MANAGE_CRAWLER_MESSAGES.S_LOCAL_URL_PARSED + " : " + m_redirected_requested_url)

                            if self.__m_host_score >= 0.90:
                                m_parsed_model, m_unique_file_model = self.__m_html_parser.on_parse_files(
                                    m_parsed_model, m_images, self.__m_proxy)

                            elastic_controller.get_instance().invoke_trigger(ELASTIC_CRUD_COMMANDS.S_INDEX,
                                                                             [ELASTIC_REQUEST_COMMANDS.S_INDEX,
                                                                              [json.dumps(m_parsed_model.dict())],
                                                                              [True]])
                        else:
                            return None, None, None
                    else:
                        if not self.__m_host_duplication_validated:
                            redis_controller.get_instance().invoke_trigger(REDIS_COMMANDS.S_SET_BOOL, [
                                REDIS_KEYS.HOST_LOW_YIELD_COUNT + p_request_model.m_url, True, 60 * 60 * 24 * 5])
                        log.g().w(str(self.__task_id) + " : " + str(
                            self.__m_tor_id) + " : " + MANAGE_CRAWLER_MESSAGES.S_LOW_YIELD_URL + " : " + m_redirected_requested_url + " : " + str(
                            m_parsed_model.m_validity_score))
                ...
                log.g().e(str(self.__task_id) + " : " + str(
                    self.__m_tor_id) + " : " + MANAGE_CRAWLER_MESSAGES.S_LOCAL_URL_PARSED_FAILED + " : " + p_request_model.m_url + " : " + str(
                    m_raw_html))
                return None, None, None
    ...
    def start_crawler_instance(self, p_request_url, p_task_id):
        ...
        while len(self.__m_unparsed_url) > 0:
            ...
            if m_parsed_model:
                if item.m_depth < CRAWL_SETTINGS_CONSTANTS.S_MAX_ALLOWED_DEPTH and len(
                        self.__m_unparsed_url) < CRAWL_SETTINGS_CONSTANTS.S_MAX_HOST_QUEUE_SIZE:
                    for sub_url in m_parsed_model.m_sub_url[
                                   0:int(CRAWL_SETTINGS_CONSTANTS.S_MAX_SUBHOST_QUEUE_SIZE / (item.m_depth + 1))]:
                        self.__m_unparsed_url.append(url_model_init(sub_url, item.m_depth + 1))

                mongo_controller.get_instance().invoke_trigger(MONGO_CRUD.S_UPDATE, [MONGODB_COMMANDS.S_UPDATE_INDEX, [
                    helper_method.on_clean_url(helper_method.get_host_url(item.m_url)), self.__m_parsed_url,
                    self.__m_unparsed_url, m_unique_file_model], [True]])
        # else:
        #    sleep(30)
...
def genbot_instance(p_url, p_vid):
    try:
        ...
        mongo_controller.get_instance().invoke_trigger(MONGO_CRUD.S_UPDATE,
                                                       [MONGODB_COMMANDS.S_CLOSE_INDEX_ON_COMPLETE, [p_request_url],
                                                        [True]])
