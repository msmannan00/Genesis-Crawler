...
class webRequestManager:
  ...
  def fetch(self, p_url, p_proxy, headers):
    try:
      ...
      response = requests.get(p_url, headers=headers, proxies=proxies, timeout=120)
  ...
  def load_header(self, p_url, p_custom_proxy):
    ...
    try:
      ...
      with m_request_handler.head(p_url, headers=headers, timeout=(CRAWL_SETTINGS_CONSTANTS.S_HEADER_TIMEOUT, 30), proxies=p_custom_proxy, allow_redirects=True, verify=False) as page:
        m_request_handler.close()
        gc.collect()
