...
from crawler.constants.strings import MANAGE_MONGO_MESSAGES, STRINGS
...
class mongo_controller(request_handler):
    ...
    def __read(self, p_data, p_limit):
        try:
            m_specific_value = STRINGS.S_EMPTY
            if MONGODB_KEYS.S_VALUE in p_data:
                m_specific_value = p_data[MONGODB_KEYS.S_VALUE]
            if p_limit is not None:
                documents = self.__m_connection[p_data[MONGODB_KEYS.S_DOCUMENT]].find(p_data[MONGODB_KEYS.S_FILTER], m_specific_value).limit(p_limit)
            else:
                documents = self.__m_connection[p_data[MONGODB_KEYS.S_DOCUMENT]].find(p_data[MONGODB_KEYS.S_FILTER], m_specific_value)
            return documents
        except Exception as ex:
