...
from crawler.constants.constant import RAW_PATH_CONSTANTS, CLASSIFIER_CONSTANTS, CRAWL_SETTINGS_CONSTANTS
...
class topic_classifier_model(request_handler):
    ...
    def __classifier_exists(self):
        if self.__m_classifier_trained is not True:
            if os.path.exists(RAW_PATH_CONSTANTS.S_PROJECT_PATH + CLASSIFIER_CONSTANTS.S_VECTORIZER_PATH) is True and \
               os.path.exists(RAW_PATH_CONSTANTS.S_PROJECT_PATH + CLASSIFIER_CONSTANTS.S_SELECTKBEST_PATH) is True:
    ...
    def __load_classifier(self):
        if self.__m_loading is False:
            ...
            self.__m_vectorizer = pickle.load(open(RAW_PATH_CONSTANTS.S_PROJECT_PATH + CLASSIFIER_CONSTANTS.S_VECTORIZER_PATH, 'rb'))
            self.__m_feature_selector = pickle.load(open(RAW_PATH_CONSTANTS.S_PROJECT_PATH + CLASSIFIER_CONSTANTS.S_SELECTKBEST_PATH, 'rb'))
            self.__m_classifier = pickle.load(open(RAW_PATH_CONSTANTS.S_PROJECT_PATH + CLASSIFIER_CONSTANTS.S_CLASSIFIER_PICKLE_PATH, 'rb'))
            self.__m_loading = False
