...
from genesis_crawler_services.crawler_services.content_duplication_manager.content_duplication_controller import content_duplication_controller
from genesis_crawler_services.crawler_services.content_duplication_manager.content_duplication_enums import CONTENT_DUPLICATION_MANAGER
...
class crawl_controller(request_handler):
    ...
    def __install_live_url(self):
        try:
            mongo_controller.get_instance().invoke_trigger(MONGODB_COMMANDS.S_RESET_CRAWLABLE_URL, None)
            m_response = requests.get(CRAWL_SETTINGS_CONSTANTS.S_START_URL)

            for line in m_response.text.splitlines():
                mongo_controller.get_instance().invoke_trigger(MONGODB_COMMANDS.S_INSTALL_CRAWLABLE_URL, line)
        except Exception as ex:
            pass

    def __init_live_url(self):
        log.g().i(INFO_MESSAGES.S_REINITIALIZING_CRAWLABLE_URL)
        m_response = mongo_controller.get_instance().invoke_trigger(MONGODB_COMMANDS.S_GET_CRAWLABLE_URL_DATA)
    ...
    def __init_thread_manager(self):
        while True:
            ...
            if app_status.CRAWL_STATUS.S_QUEUE_BACKUP_STATUS is False and len(self.__m_crawler_instance_list)<=0:
                mongo_controller.get_instance().invoke_trigger(MONGODB_COMMANDS.S_CLEAR_CACHE)
    ...
    def __crawler_instance_manager(self):
        for m_crawl_instance in self.__m_crawler_instance_list:
            ...
            if m_thread_status == CRAWLER_STATUS.S_STOP:
                ...
                mongo_controller.get_instance().invoke_trigger(MONGODB_COMMANDS.S_UPDATE_CRAWLABLE_URL_DATA, [m_request_model.m_url, m_url_status])
