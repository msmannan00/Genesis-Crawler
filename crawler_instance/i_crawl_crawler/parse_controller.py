...
from crawler_services.crawler_services.content_duplication_manager.content_duplication_controller import \
...
class parse_controller:
    ...
    def on_parse_html(self, p_html, p_base_url_model):
        m_title, m_description, m_uniary_tfidf_score, m_binary_tfidf_score, m_validity_score,m_content_type,  m_sub_url, m_images, m_doc, m_vid = self.__on_html_parser_invoke(p_base_url_model, p_html)
        ...
        if m_duplication_status is False:
            if m_content_type is None:
                m_content_type = CRAWL_SETTINGS_CONSTANTS.S_THREAD_CATEGORY_UNKNOWN
